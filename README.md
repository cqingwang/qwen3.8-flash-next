...
本地工作区是源码源。`install` 会先将项目同步到 head 和 worker，再从 head 调用 `dspark/start.sh`。模型不会复制进项目目录；子模块的本地模型模式会把 head 的模型目录断点 rsync 到 worker，并在容器中以 `/models/Qwen3.8-Flash-Next-NVFP4` 挂载。
