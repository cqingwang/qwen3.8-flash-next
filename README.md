...
接入客户段 agent 时，模型条目的 `contextWindow` 必须同步设置为 `1000000`，并在 agent 自身的 `config.yaml` 中保持 `compression.enabled: true`。上下文数值的唯一源头是本文件的 `config.yaml`，`program.py --install`/`--gen-env` 会将其完整生成到 `dspark/.env`；`dspark/README.md` 中的 agent 片段只是同步示例。后者属于 agent 会话配置，不由本仓库的 SGLang 部署配置读取。修改后需要新建 agent 会话，旧会话不会自动缩短已经累积的历史。
